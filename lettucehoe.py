...
def plantmask(im, ws_par, ts, bilf=[11, 5, 17], morpho_it=[5,5]):
   im_ws=im[ws_par[1]-ts/2.:ws_par[1]+ws_par[3]+ts,ws_par[0]-ws_par[2]-ts/2.:ws_par[0]+ts,:]
   cidx=np.nan_to_num(exgreen(im_ws))
   M=cidx.max()
   m=cidx.min()
...
def mod_boustrophedon(omask, im, ts, ws_par, eps_contours=1, Nfp=5000, plotit=None):
   #Compute boustrophedon ignoring the plants and transform to workspace coordinates
   boustro=plain_boustrophedon(ws_par[3]/ts, ws_par[2], ts, ts/2., ts/2.)
   boustro[0]*=-1
   boustro[0]+=omask.shape[1]

   #Detect i/o points of paths passing through plants
   dense_boustro=fillNpoints(boustro,Nfp)
   idxs=np.where(np.diff(omask[(dense_boustro[1]).astype(int),(dense_boustro[0]).astype(int)])>0)[0]
